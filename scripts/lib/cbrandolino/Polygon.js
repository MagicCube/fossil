...
{
    return {
        x1: Math.min.apply(Math,this.vertices.map(function(o){return o.lng;})),
        x2: Math.max.apply(Math,this.vertices.map(function(o){return o.lng;})),
        y1: Math.min.apply(Math,this.vertices.map(function(o){return o.lat;})),
        y2: Math.max.apply(Math,this.vertices.map(function(o){return o.lat;}))
    };
};

Polygon.prototype.getConvexPolygon = function()
{
    this.vertices.sort(sortPointY);
    this.vertices.sort(sortPointX);
    var hullPoints = [];
    chainHull_2D(this.vertices, this.vertices.length, hullPoints);
